...
# Enhanced script that not only detects JDK 25 in Jenkinsfiles but also:
# - Finds the commit that added JDK 25
# - Finds the PR associated with that commit
...
echo "========================================" > "$LOG_FILE"
echo "JDK 25 Tracking with PR Detection" >> "$LOG_FILE"
...
find_jdk25_commit() {
  ...
  # Look for patterns like: jdk: '25', java: 25, openjdk-25, etc.
  local commit_sha=$(git log -G "25" --all --format="%H" --follow -- Jenkinsfile | while read sha; do
    # Check if this commit actually added JDK 25 (not just contains the number 25)
...
info "Starting JDK 25 detection with PR tracking in Jenkins plugin Jenkinsfiles..."
...
# (parallel git clones can be problematic)
while read -r repo_path; do
  check_jdk25_with_pr "$repo_path"
done < "$repos_list"
